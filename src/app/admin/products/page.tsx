...
const BLANK_P = { name: "", description: "", categoryId: "", basePrice: "" };
const BLANK_V = { color: "", dimensions: "", stock: "" };
...
export default function AdminProductsPage() {
  ...
  function openEditProduct(p: Product) { setEditProd(p); setPForm({ name: p.name, description: p.description ?? "", categoryId: p.categoryId, basePrice: p.basePrice?.toString() ?? "" }); setShowPForm(true); }
  ...
  async function saveProduct() {
    ...
    setSavingP(true);
    try {
      const d = { name: pForm.name, description: pForm.description || undefined, categoryId: pForm.categoryId, basePrice: pForm.basePrice ? parseFloat(pForm.basePrice) : undefined };
      if (editProd) { await api.products.update(editProd.id, d); toast.success("Updated"); }
      else          { await api.products.create(d); toast.success("Created"); }
      closePForm(); load();
  ...
  function openEditVariant(v: ProductVariant) { setEditVar(v); setVForm({ color: v.color ?? "", dimensions: v.dimensions ?? "", stock: v.stock?.toString() ?? "" }); setShowVForm(v.productId); }
  ...
  async function saveVariant() {
    ...
    setSavingV(true);
    try {
      const d = { color: vForm.color || undefined, dimensions: vForm.dimensions || undefined, stock: vForm.stock ? parseInt(vForm.stock) : undefined };
      if (editVar) { await api.variants.update(editVar.productId, editVar.id, d); toast.success("Updated"); }
      else         { await api.variants.create(showVForm, d); toast.success("Added"); }
      closeVForm(); load();
    } catch (err: any) { toast.error(err.message); }
    ...
    <div className="space-y-5 fade-up">
        ...
        <div className="p-5 slide-in space-y-4" style={{ backgroundColor: "#FDFAF6", border: "1px solid #C8913A" }}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input value={pForm.name} onChange={(e) => setPForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Chiavari Chair" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Category *</label>
              <select value={pForm.categoryId} onChange={(e) => setPForm((f) => ({ ...f, categoryId: e.target.value }))} style={{ ...inputStyle, cursor: "pointer" }}>
            ...
            <div>
              ...
              <input type="number" value={pForm.basePrice} onChange={(e) => setPForm((f) => ({ ...f, basePrice: e.target.value }))} placeholder="e.g. 150" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Description</label>
              <input value={pForm.description} onChange={(e) => setPForm((f) => ({ ...f, description: e.target.value }))} placeholder="Short description" style={inputStyle} />
            </div>
          </div>
      ...
      {loading ? (
        <div className="space-y-px">
          {Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-16 animate-pulse" style={{ backgroundColor: "#EDE9E1" }} />)}
        </div>
      ) : products.length === 0 ? (
            ...
            <div key={p.id}>
              <div className="flex items-center gap-3 px-4 py-3.5 transition-colors hover:bg-[#F2EFE9]"
                ...
                <button onClick={() => setExpanded(expanded === p.id ? null : p.id)}
                  className="shrink-0 transition-transform" style={{ color: "#D4C9B0" }}>
                  {expanded === p.id
              ...
              {expanded === p.id && (
                <div className="px-10 py-4" style={{ backgroundColor: "#F2EFE9", borderBottom: "1px solid #EAE3D2" }}>
                  ...
                  {showVForm === p.id && (
                    <div className="mb-3 p-4 space-y-3" style={{ backgroundColor: "#FDFAF6", border: "1px solid #C8913A" }}>
                      <div className="grid gap-3 sm:grid-cols-3">
                        {[
                          { k: "color",      label: "Color",      ph: "Black" },
                          { k: "dimensions", label: "Dimensions", ph: "45x45x90 cm" },
                          { k: "stock",      label: "Stock",      ph: "50" },
                        ].map(({ k, label, ph }) => (
                          <div key={k}>
                            <label style={labelStyle}>{label}</label>
                            <input value={(vForm as any)[k]}
                              onChange={(e) => setVForm((f) => ({ ...f, [k]: e.target.value }))}
                              placeholder={ph} type={k === "stock" ? "number" : "text"}
                              style={inputStyle} />
                          </div>
                        ))}
                      </div>
                  ...
                  {p.variants && p.variants.length > 0 ? (
                    ...
                    <p className="text-xs" style={{ color: "#D4C9B0" }}>No variants added yet.</p>
                  )}
                </div>
              )}
            </div>
