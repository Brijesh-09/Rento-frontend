...
import { Trash2, Plus, Minus, ArrowLeft, CheckCircle, ArrowRight } from "lucide-react";
  ...
  if (submitted) {
    return (
      <div className="max-w-xl mx-auto px-6 py-24 text-center fade-up">
        ...
        <h1 className="font-display text-3xl mb-3" style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}>Quote Submitted</h1>
        <p className="mb-2" style={{ color: "#8A8680", lineHeight: 1.7 }}>
        ...
        <div className="flex gap-3 justify-center">
          <Link href="/products"
            className="px-6 py-3 text-sm uppercase tracking-wider font-medium"
            style={{ backgroundColor: "#1C1C1A", color: "#F7F4EF", letterSpacing: "0.08em" }}>
            Browse More
          </Link>
          <button onClick={() => { setSubmitted(false); setForm({ name:"",email:"",phone:"",eventName:"",eventType:"",location:"",startDate:"",endDate:"",notes:"" }); }}
    ...
    <div>
      ...
      <section style={{ backgroundColor: "#EDE9E1", borderBottom: "1px solid #D4C9B0" }}>
        <div className="max-w-7xl mx-auto px-6 py-10">
          <Link href="/products" className="inline-flex items-center gap-2 text-xs uppercase tracking-wider mb-4 transition-colors hover:text-[#C8913A]"
            style={{ color: "#8A8680", letterSpacing: "0.1em" }}>
            <ArrowLeft style={{ width: 12, height: 12 }} /> Back to products
          </Link>
          <h1 className="font-display text-4xl" style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}>Request a Quote</h1>
          <p className="mt-2 text-sm" style={{ color: "#8A8680" }}>
            Review your items, fill in your event details, and we'll get back with personalised pricing.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 grid lg:grid-cols-2 gap-12">
        ...
        <div>
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-display text-xl" style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}>
              Your Items
            </h2>
          ...
          {items.length === 0 ? (
            <div className="py-16 text-center" style={{ border: "1px dashed #D4C9B0" }}>
              <p className="font-display text-lg mb-2" style={{ fontFamily: "'DM Serif Display', Georgia, serif", color: "#8A8680" }}>
                Your list is empty
              </p>
              <Link href="/products"
                className="inline-flex items-center gap-1 text-xs uppercase tracking-wider mt-3 transition-colors hover:text-[#C8913A]"
                style={{ color: "#8A8680", letterSpacing: "0.1em" }}>
              ...
              {items.map((item) => (
                <div key={`${item.productId}-${item.variantId}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate" style={{ color: "#1C1C1A" }}>{item.productName}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          {/* Contact */}
          <div>
            <p className="font-display text-xl mb-5" style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}>Contact Details</p>
          ...
          {/* Event */}
          <div>
            <p className="font-display text-xl mb-5" style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}>Event Details</p>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label style={labelStyle}>Event Name</label>
                <input value={form.eventName} onChange={set("eventName")} placeholder="Annual Sales Meet" style={inputStyle} />
              </div>
              <div>
          ...
          <button type="submit" disabled={submitting || items.length === 0}
            className="w-full flex items-center justify-center gap-2 py-4 text-sm font-medium uppercase tracking-wider transition-all disabled:opacity-40"
          ...
          <p className="text-xs text-center" style={{ color: "#8A8680" }}>
            Our team typically responds within 2–4 hours on business days.
          </p>
        </form>
      </div>
    </div>
