...
const HERO_SLIDES = [
  ...
  {
    img: "https://images.unsplash.com/photo-1519225421980-715cb0215aed?w=1920&q=90",
    tag: "Weddings & Receptions",
    headline: ["Your dream", "wedding,", "beautifully set."],
    sub: "Elegant seating, tables and décor delivered to your venue.",
  },
  {
    img: "https://images.unsplash.com/photo-1475721027785-f74eccf877e2?w=1920&q=90",
    tag: "Exhibitions & Expos",
    headline: ["Stand out at", "every", "exhibition."],
    sub: "Display panels, seating and tables for trade shows and expos.",
  },
];
...
const SCROLLBAR_STYLES = `
  ...
  .custom-scrollbar::-webkit-scrollbar-thumb:hover {
    background-color: rgba(28,28,26,0.3);
  }
  .custom-scrollbar {
    scrollbar-width: thin;
    scrollbar-color: rgba(28,28,26,0.15) transparent;
  }
`;
...
const BANNER_ITEMS = [
  {
    ...
    desc: "Conference tables, ergonomic chairs, display boards — everything you need for a productive meeting.",
    cta: "Shop Conference",
    accent: "#3D5A4A",
    align: "left",
  },
...
export default async function HomePage() {
  ...
  return (
    <div style={{ backgroundColor: "#F7F4EF" }}>
      ...
      {/* ═══════════════════════════════════════════════════════════════
          HERO — Immersive full-viewport with animated headline
      ═══════════════════════════════════════════════════════════════ */}
      <section className="relative" style={{ height: "100svh", minHeight: 640 }}>
        ...
        {/* Multi-stop gradient for rich dark-to-transparent feel */}
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-10 md:px-20 max-w-7xl" style={{ margin: "0 auto", left: 0, right: 0 }}>
          <div className="max-w-3xl">
            ...
            <div className="inline-flex items-center gap-2 mb-8 fade-up">
              ...
              <span className="text-[10px] uppercase tracking-[0.22em]" style={{ color: "#C8913A" }}>
                {hero.tag}
              </span>
            </div>
            ...
            <div className="fade-up-4 flex flex-wrap gap-3">
              <Link href="/products"
                ...
                Browse Collection
                <ArrowRight style={{ width: 16, height: 16 }} className="transition-transform group-hover:translate-x-1" />
              </Link>
              <Link href="/quote"
                className="group inline-flex items-center gap-3 px-8 py-4 text-sm font-medium uppercase transition-all duration-300"
        ...
        <div className="absolute bottom-0 left-0 right-0" style={{ backgroundColor: "rgba(12,12,10,0.82)", backdropFilter: "blur(12px)" }}>
          <div className="max-w-7xl mx-auto px-8 py-5 grid grid-cols-2 md:grid-cols-4 gap-4 divide-x divide-[rgba(247,244,239,0.08)]">
            {TRUST.map((t, i) => (
              <div key={t.label} className={`flex flex-col items-center ${i > 0 ? "pl-4" : ""}`}>
      ...
      {/* ═══════════════════════════════════════════════════════════════
          CATEGORY STRIP — Big tiles, horizontal scroll, real photos
      ═══════════════════════════════════════════════════════════════ */}
      {categories.length > 0 && (
        <section className="py-16" style={{ backgroundColor: "#F7F4EF" }}>
          <div className="max-w-7xl mx-auto">
            ...
            <div className="px-6 md:px-10 flex items-end justify-between mb-8">
              ...
              <Link href="/products"
                className="hidden md:inline-flex items-center gap-1.5 text-xs uppercase tracking-wider transition-colors hover:text-[#C8913A]"
            ...
            {/* Scrollable tiles */}
            <AutoScroller speed={40} className="flex gap-3 overflow-x-auto px-6 md:px-10 pb-6 custom-scrollbar">
              {/* "All" tile */}
              <Link href="/products"
                className="group relative shrink-0 overflow-hidden flex flex-col justify-end"
                style={{ width: 160, height: 340, backgroundColor: "#1C1C1A", scrollSnapAlign: "start", flexShrink: 0 }}>
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div className="w-12 h-12 rounded-full border mx-auto mb-3 flex items-center justify-center transition-all group-hover:border-[#C8913A]"
                ...
                <div className="relative p-4">
                  <p style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: 20, color: "#F7F4EF", lineHeight: 1.2 }}>
                    All<br />Items
                  </p>
              ...
              {categories.map((cat: any) => {
                ...
                return (
                  <Link key={cat.id} href={`/products?categoryId=${cat.id}`}
                    ...
                    style={{ width: 260, height: 340, scrollSnapAlign: "start", flexShrink: 0 }}>
                    {/* Photo */}
                    ...
                    <div className="absolute top-0 left-0 right-0 h-0.5 transition-all duration-300 scale-x-0 group-hover:scale-x-100 origin-left"
      ...
      {/* ═══════════════════════════════════════════════════════════════
          TRENDING PRODUCTS CAROUSEL — Rich cards with real images
      ═══════════════════════════════════════════════════════════════ */}
      {
        products.length > 0 && (
          <section className="py-16" style={{ backgroundColor: "#EDE9E1" }}>
            <div className="max-w-7xl mx-auto">
              ...
              {/* Product scroll */}
              <AutoScroller speed={50} className="flex gap-4 overflow-x-auto px-6 md:px-10 pb-6 custom-scrollbar">
                {products.map((p: any, i: number) => {
                  ...
                  return (
                    <div key={p.id}
                      className="group shrink-0 flex flex-col cursor-pointer"
                      style={{ width: 240, scrollSnapAlign: "start", flexShrink: 0, backgroundColor: "#FDFAF6" }}>
                      ...
                      <div className="relative overflow-hidden" style={{ height: 260, backgroundColor: bg }}>
                        ...
                        <div className="absolute top-3 left-3 px-2.5 py-1 text-[9px] uppercase tracking-wider font-medium"
                          style={{ backgroundColor: "rgba(247,244,239,0.92)", color: "#8A8680", backdropFilter: "blur(4px)" }}>
                          {p.category?.name}
                        </div>
      ...
      {/* ═══════════════════════════════════════════════════════════════
          FEATURED BANNERS — Full-bleed alternating editorial sections
      ═══════════════════════════════════════════════════════════════ */}
      <section className="py-16" style={{ backgroundColor: "#F7F4EF" }}>
          ...
          <div className="flex flex-col gap-4">
            {BANNER_ITEMS.map((banner, i) => (
              <div key={i}
                ...
                style={{ height: 420, cursor: "pointer" }}>
                ...
                {/* Gradient — direction flips per banner */}
                <div className="absolute inset-0" style={{
                ...
                {/* Content */}
                <div className={`absolute inset-0 flex flex-col justify-center p-10 md:p-16 ${banner.align === "right" ? "items-end text-right" : "items-start"}`}>
                  <div className="max-w-md">
                    <div className="inline-flex items-center gap-2 mb-5">
                      <span className="text-[10px] uppercase tracking-[0.2em]" style={{ color: banner.accent }}>{banner.eyebrow}</span>
                    </div>
                    ...
                    <p className="mb-8 text-sm leading-relaxed" style={{ color: "rgba(247,244,239,0.65)", maxWidth: 360 }}>
                      {banner.desc}
                    </p>
                    <Link href="/products"
                      className="group/btn inline-flex items-center gap-3 px-7 py-3.5 text-xs font-medium uppercase transition-all"
                      style={{ backgroundColor: banner.accent, color: "#F7F4EF", letterSpacing: "0.1em" }}>
                      {banner.cta}
                      <ArrowRight style={{ width: 14, height: 14 }} className="transition-transform group-hover/btn:translate-x-1" />
                    </Link>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-4 grid md:grid-cols-2 gap-4">
            <div className="group relative overflow-hidden" style={{ height: 300 }}>
              <img src="https://images.unsplash.com/photo-1475721027785-f74eccf877e2?w=900&q=85"
                alt="Exhibition" className="absolute inset-0 w-full h-full transition-transform duration-700 group-hover:scale-105"
                style={{ objectFit: "cover" }} />
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-8">
                ...
                <Link href="/products" className="mt-4 inline-flex items-center gap-2 text-xs uppercase tracking-wider transition-colors hover:text-[#D4A59A]"
            ...
            <div className="relative overflow-hidden flex flex-col gap-0" style={{ height: 300 }}>
              <div className="relative group overflow-hidden flex-1" style={{ height: "50%" }}>
                <img src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=900&q=85"
                  alt="Cooling" className="absolute inset-0 w-full h-full transition-transform duration-700 group-hover:scale-105"
                  style={{ objectFit: "cover" }} />
                ...
                <div className="absolute inset-0 flex items-center px-8 gap-4">
                  ...
                  <Link href="/products" className="ml-auto w-8 h-8 rounded-full flex items-center justify-center shrink-0"
                    style={{ backgroundColor: "rgba(247,244,239,0.15)", color: "#F7F4EF" }}>
                    <ArrowUpRight style={{ width: 14, height: 14 }} />
                  </Link>
                </div>
              </div>
              <div className="relative group overflow-hidden flex-1" style={{ height: "50%", marginTop: 4 }}>
                <img src="https://images.unsplash.com/photo-1511578314322-379afb476865?w=900&q=85"
                  alt="All" className="absolute inset-0 w-full h-full transition-transform duration-700 group-hover:scale-105"
                  style={{ objectFit: "cover" }} />
      ...
      <section className="py-20" style={{ backgroundColor: "#1C1C1A" }}>
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          ...
          <div className="grid md:grid-cols-3 gap-px" style={{ backgroundColor: "#2E2E2B" }}>
            {[
              { n: "01", title: "Browse & Select", desc: "Pick products, quantities and variants from our catalogue. Mix and match freely across all categories.", icon: "🛋️" },
              { n: "02", title: "Submit Requirements", desc: "Tell us your event dates, venue and any special requirements. Takes less than 2 minutes.", icon: "📋" },
              { n: "03", title: "We Quote & Deliver", desc: "Our team sends a personalised quote within hours. Confirm and we handle delivery to your venue.", icon: "🚚" },
            ].map((step, i) => (
      ...
      <section className="py-16" style={{ backgroundColor: "#F7F4EF" }}>
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          <div className="mb-10">
            ...
            <h2 style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: "clamp(1.8rem,3.5vw,2.8rem)", color: "#1C1C1A" }}>
              Trusted by event<br />professionals
            </h2>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {TESTIMONIALS.map((t, i) => (
              <div key={t.name} className="p-7"
                ...
                <p className="text-sm mb-6 leading-relaxed" style={{ color: i === 1 ? "rgba(247,244,239,0.7)" : "#8A8680", fontStyle: "italic" }}>
                  "{t.text}"
                </p>
                <div className="flex items-center gap-3" style={{ borderTop: `1px solid ${i === 1 ? "#2E2E2B" : "#EAE3D2"}`, paddingTop: 18 }}>
                  <div className="w-9 h-9 rounded-sm flex items-center justify-center text-xs font-medium"
      ...
      <section className="relative overflow-hidden" style={{ height: 500 }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          ...
          <div className="flex gap-3 flex-wrap justify-center">
            <Link href="/quote"
              className="group inline-flex items-center gap-3 px-9 py-4 text-sm font-medium uppercase transition-all"
              style={{ backgroundColor: "#C8913A", color: "#F7F4EF", letterSpacing: "0.1em" }}>
              Request a Quote <ArrowRight style={{ width: 15, height: 15 }} className="transition-transform group-hover:translate-x-1" />
            </Link>
            <Link href="/products"
